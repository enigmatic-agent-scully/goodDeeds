...
class Main extends Component {
  constructor() {
    ...
    this.handleSaveToExistingProfile = this.handleSaveToExistingProfile.bind(
      this
    );
  ...
  uploadHandler(event) {
    ...
    uploadFile(imagefile, this.reactS3config)
      .then(res => {
        ...
        console.log(
          'res.location after returning from uploadHandler promise' +
            res.location
        );
  ...
  handleSaveToExistingProfile(event) {
    ...
    Auth.updateUserInfo(SignUpInfo)
      .then(res => {
        ...
        console.log(
          'res after returning from hadnleSavetoExistingProfile promise' + res
        );
      })
      .catch(err => {
        console.log(err);
      ...
      <div className='Main-Page'>
        <div className='navbar-wrapper'>
          <Navbar id='navbar' brand='goodDeeds' fixed right>
            <Dropdown trigger={<Button>{this.state.userInfo.userName}</Button>}>
              <Modal
                id='profile-modal'
                open={this.state.isModalOpen}
                trigger={
                  <NavItem onClick={this.getProfileInfo}>Edit Profile</NavItem>
                }
              >
                <ProfileView
            ...
            <NavItem href='/main/get-help' className='hover-over-nav'>
              Get Help
            </NavItem>
            <NavItem href='/main/give-help' className='hover-over-nav'>
              Give Help
            </NavItem>
          </Navbar>
        </div>
        <Router>
          <div className='main-body'>
            ...
            <Route
              ...
              path='/main/give-help'
              render={props => (
                <GiveHelp {...props} user={this.state.userInfo} />
              )}
            />
          </div>
        </Router>
